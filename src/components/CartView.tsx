  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="p-4 rounded-xl border border-gray-200 bg-white flex flex-col gap-3 sm:w-80 sm:ms-auto">
        ...
        {/* Delivery, here, before the customer commits to anything.

            This box used to show the subtotal twice — once labelled
            "Sous-total" and once labelled "Total" — and then checkout added
            8 DT. On a small basket that is the price jumping by half at the
            last step, which is the most reliable way there is to lose an
            order that was already won. The fee is flat and the same anywhere
            in Tunisia, so nothing here has to be guessed. */}
        <div className="flex justify-between text-sm text-gray-600">
        ...
        <div className="flex justify-between font-bold text-navy-900 text-lg pt-1 border-t border-gray-100">
          ...
          <Price value={delivery.total} />
        </div>

        {/* Only ever quotes the home-delivery figure, which can go down at
            checkout (pickup is free) and never up. */}
        {delivery.remainingForFree > 0 ? (
