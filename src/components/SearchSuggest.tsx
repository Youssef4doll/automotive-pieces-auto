          ...
          <li key={`${s.kind}-${s.href}-${s.label}`} id={`${listId}-${i}`} role="option" aria-selected={i === active}>
            ...
            >
              <span className="flex-1 min-w-0">
                <span className="block text-[15px] text-navy-950 leading-tight truncate">{s.label}</span>
                {s.hint && <span className="block text-[13px] text-gray-500 truncate">{s.hint}</span>}
              </span>
              <span className="shrink-0 text-[12px] uppercase tracking-wide text-gray-600">
                {KIND_LABEL[s.kind]}
              </span>
            </button>
          </li>
