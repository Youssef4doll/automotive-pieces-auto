        ...
        return (
          <li key={step.key} className="flex items-start flex-1 last:flex-none min-w-0">
            <div className="flex flex-col items-center gap-1 min-w-0">
              ...
              <span className={`text-[12px] whitespace-nowrap ${done ? "text-green-700 font-semibold" : "text-gray-600"}`}>
                {step.label}
              </span>
              {/* Reserve the line whether or not there is a date, so the four
                  steps stay on one baseline instead of jumping about. */}
              <span className="text-[11px] leading-tight text-center text-gray-600 whitespace-nowrap min-h-[2.1em]">
                {moment ? (
                  <>
                    {moment.day}
                    <br />
