    ...
    <div dir="ltr" lang="fr" className="min-h-screen flex bg-gray-50 text-navy-950">
      <aside className="hidden lg:flex flex-col w-64 shrink-0 bg-navy-950 text-white border-b-2 border-gold-500">
        <div className="p-5 border-b border-white/10 flex flex-col gap-1.5">
          ...
          <span className="font-display font-bold uppercase text-[12px] tracking-[0.16em] text-gold-500">
            Espace admin
          </span>
        </div>
        <AdminNav />
        <div className="mt-auto p-4 border-t border-white/10 flex flex-col gap-2">
          <Link href="/" className="font-display font-semibold uppercase text-[12px] tracking-wide text-white/50 hover:text-gold-500">
            ← Retour au site
          </Link>
          <form action={logout}>
            <button className="font-display font-semibold uppercase text-[12px] tracking-wide text-white/50 hover:text-gold-500">
              Déconnexion
            </button>
          </form>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="lg:hidden bg-navy-950 text-white p-4 flex items-center justify-between border-b-2 border-gold-500">
          <Image src="/images/logo-white.png" alt="" width={130} height={43} className="h-7 w-auto" />
          <Link href="/" className="font-display font-semibold uppercase text-[12px] tracking-wide text-white/60">
            Retour au site
          </Link>
        </header>
